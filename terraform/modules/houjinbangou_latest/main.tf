resource "google_project_service" "houjinbangou_latest" {
  for_each = toset([
...
resource "google_workflows_workflow" "houjinbangou_latest" {
...
resource "google_cloud_scheduler_job" "houjinbangou_latest" {
  name      = "houjinbangou_latest"
  ...
  http_target {
    uri         = "https://workflowexecutions.googleapis.com/v1/${google_workflows_workflow.houjinbangou_latest.id}/executions"
    http_method = "POST"
    oauth_token {
...
resource "google_cloudbuild_trigger" "houjinbangou_latest" {
